...
class="navbar navbar-light" style="background-color: #e3f2fd;">
       <form action='' method='GET'>
                ...
                <?php
                    $result2 = pg_query($db, "select * from profile natural join record inner join doctor on record.doctor = doctor.id");
		            $rows2 = pg_num_rows($result2);?>
           	    <?php echo ('Предстоящих записей: '.$rows2)?>
                </span>
                <font size="+2">
  ...
  <table class="table table-lg">
    ...
    <?php
        $query = 'select record.id, record.doctor,time, date, doctor.first_name, doctor.last_name, doctor.type from profile natural join record inner join doctor on record.doctor = doctor.id';
    ...
    <tbody>
        ...
        <form action='' method="GET" role = "form">
            ...
            <a href="deleterecord.php?id=<?php echo $row['doctor']; ?>" name='deleterecord' class="btn btn-danger"> Отменить запись </a>
        </form>
        </td>
        </tr>
    </tbody>
